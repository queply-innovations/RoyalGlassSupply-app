				...
				<PendingInventoryProductProvider>
					<div className="flex h-full flex-1 flex-col gap-5 rounded-xl border border-black/10 bg-white p-4">
						<div className="max-h-full w-full flex-1 rounded-md border">
							<PendingInventoryProductTable openModal={modalHandler} />
						</div>
					</div>
					<ModalTest
						title={
					...
					>
						<>
							...
							{modalAction === 'edit' && (
								<EditPendingInventoryProductForm onClose={closeModal} />
							)}
							{modalAction === 'approve' && (
								<ApproveInventoryProductPending onClose={closeModal} />
							)}
						</>
					</ModalTest>
				</PendingInventoryProductProvider>
